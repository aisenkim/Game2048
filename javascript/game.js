...
S_game.canKeyPress = true; //애니메이션 도중 키 toggle 
...
S_game.initGameBoard = function() { //게임 보드에다가 줄 만들어 주기위해 16개 div 상자 만들어 주기 
    for(var i=0; i<4; i++){
        for(var j=0; j<4; j++){
...
S_game.createBlock = function () {
    ...
    //======================================================================================================================
    var options = []; //gArray 배열 돌면서 비어있는 값들만 넣어둔다
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 4; j++) {
    ...
    if (options.length > 0) { //비어있는 자리가 있을경우 
        var spot = UTIL.randomArray(options); //비어있는 블럭을 돌려받은것
...
S_game.updateBoard = function () {
    ...
    setTimeout(function () {
        ...
        for (var i = 0; i < 4; i++) { //보드에 있는 블럭들 다 제거 해주는 부분 
            for (var j = 0; j < 4; j++) {
                $('#block' + i + j).remove();
                S_game.blockOnScreen = 0; //화면에 있는 블락 수 다 지워준다
            }
...
//after postion 이 현재 position 말하는거
...
S_game.onclick = function () {
    ...
    $("#button-left").unbind().click(function () {
        //애니메이션 중 키를 막기위해서 toggle 설정함.
        if (S_game.canKeyPress == false) return;
        S_game.moveLeft();
        S_game.updateBoard();
        S_game.updateScore();
        return;
